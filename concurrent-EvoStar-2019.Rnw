...
parameters of a population based concurrent evolutionary algorithm that yields to
...
Onemax, the generation gap that seems to be more appropriate is around 16. The time to
...
channel, and this implies a certain overhead in reading, transmitting
...
overcome. Also we have to somehow address the problem of the message
size during communication to and from the channel; in this respect, we are planning to resort to a compact representation of the population, obtained either by a statistical distribution model or  a standard compression algorithm. This is left as an avenue for future work.
...
\section{Acknowledgements}
 ...
 helped us with the adventure of programming efficient concurrent
